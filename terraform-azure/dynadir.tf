resource "azurerm_public_ip" "dynadir" {
  name                    = "dynadir"
...
resource "azurerm_network_interface" "dynadirnetwork" {
  name                = "dynadir-nic"
  location            = azurerm_resource_group.allinonerg.location
  resource_group_name = azurerm_resource_group.allinonerg.name
  ...
  ip_configuration {
    ...
    public_ip_address_id          = azurerm_public_ip.dynadir.id
  }
}

resource "azurerm_windows_virtual_machine" "dynadir" {
  name                  = "dynadir"
  location              = azurerm_resource_group.allinonerg.location
  resource_group_name   = azurerm_resource_group.allinonerg.name
  network_interface_ids = [azurerm_network_interface.dynadirnetwork.id]
  size                  = "Standard_B2s"
